...
def test_upload_txt_file(test_txt_file_path):
    ...
    assert response.status_code == 200
    assert response.json() == {"message": "Document 'test_upload.txt' processed and added successfully."}
...
def test_upload_unsupported_file():
    ...
    response = client.post("/upload", files={"file": ("test_unsupported.docx", dummy_content, "application/vnd.openxmlformats-officedocument.wordprocessingml.document")})
...
def test_query_after_upload(test_txt_file_path):
    ...
    with open(test_txt_file_path, "rb") as f:
         client.post("/upload", files={"file": ("test_upload.txt", f, "text/plain")}) # Re-upload for test isolation if needed
