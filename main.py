...
app = FastAPI(title="DocumentAgent API", description="API for uploading documents and querying them using RAG", lifespan=lifespan)
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        if processing_success:
            ...
            return JSONResponse(status_code=200, content={"message": f"Document '{file.filename}' processed and added successfully."})
        else:
            # If add_document_to_vector_store returned False explicitly
            print("Processing reported failure.")
            raise HTTPException(status_code=500, detail=f"Failed to process document '{file.filename}'. Check server logs for errors during processing.")
        ...
        raise HTTPException(status_code=500, detail=f"An unexpected error occurred in API endpoint: {e}")
    finally:
